function [ f_opt,X_opt,z_opt,CPUTime] = WrittenExamNewUB2CombinedProductionExp21( ZIGMAtemp,mu,m1,m2,N,switch1,R0,gamma1,gamma2,cosiPrime,c,d,alpha_k,beta_k,m,n)
...
variables q(1,size) zeta(size,N) y(N) s11(N) s21(N) s12(N) s22(N) s13(N) s23(N) s14(N) s24(N) s15(N) s25(N);
...
%%
minimize( c'*x + Landa*R0 + gamma2*(sum(sum(eye(m1).*Q1)) + sum(sum(eye(m2).*Q2))) + sqrt(gamma1)*norm(q,2) + (1/N) * sum(y) );
%%
subject to
