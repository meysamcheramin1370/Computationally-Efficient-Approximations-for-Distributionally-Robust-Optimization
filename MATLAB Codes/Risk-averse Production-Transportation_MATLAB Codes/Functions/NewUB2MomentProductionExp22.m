function [ f_opt,X_opt,z_opt,CPUTime ] = NewUB2MomentProductionExp22( ZIGMAtemp,mu,A,b,m1,m2,m3,gamma1,gamma2,switch1,c,d,alpha_k,beta_k,m,n)
%This function finds the new reformulation of 3-partition upper bound (General moment-based ambiguity
...
warning off;
                ...
                variables q(size) s s11 s21 s31 s12 s22 s32 s13 s23 s33 s14 s24 s34 s15 s25 s35;
                ...
                %%
                minimize( c'*x + s + sum(sum((gamma2*eye(m1)).*Q1))+ sum(sum((gamma2*eye(m2)).*Q2))+ sum(sum((gamma2*eye(m3)).*Q3))+ sqrt(gamma1)*norm(q,2) );
                %%
                subject to
                ...
                s11 + s21 + s31 == s- beta_k(1) - alpha_k(1)*z1'*mu - Landa1'*(b-A*mu);
                s12 + s22 + s32 == s- beta_k(2) - alpha_k(2)*z2'*mu - Landa2'*(b-A*mu);
                s13 + s23 + s33 == s- beta_k(3) - alpha_k(3)*z3'*mu - Landa3'*(b-A*mu);
                s14 + s24 + s34 == s- beta_k(4) - alpha_k(4)*z4'*mu - Landa4'*(b-A*mu);
                s15 + s25 + s35 == s- beta_k(5) - alpha_k(5)*z5'*mu - Landa5'*(b-A*mu);
