function [ f_opt,X_opt,z_opt,CPUTime] = WrittenExamNewUB2CombinedProductionExp24( ZIGMAtemp,mu,m1,m2,m3,m4,m5,N,switch1,R0,gamma1,gamma2,cosiPrime,c,d,alpha_k,beta_k,m,n)
...
[ A1Complete,A1,A2,A3,A4,A5,ZIGMA,U,delta ] = New4covTransformerDecomposer(ZIGMAtemp,size,m1,m2,m3,m4,m5,switch1);
...
variables q(1,size) zeta(size,N) y(N) s11(N) s21(N) s31(N) s41(N) s51(N) s12(N) s22(N) s32(N) s42(N) s52(N) s13(N) s23(N) s33(N) s43(N) s53(N) s14(N) s24(N) s34(N) s44(N) s54(N) s15(N) s25(N) s35(N) s45(N) s55(N);
...
%%
minimize( c'*x + Landa*R0 + gamma2*(sum(sum(eye(m1).*Q1)) + sum(sum(eye(m2).*Q2)) + sum(sum(eye(m3).*Q3)) + sum(sum(eye(m4).*Q4)) + sum(sum(eye(m5).*Q5))) + sqrt(gamma1)*norm(q,2) + (1/N) * sum(y) );
%%
subject to
...
for i = 1 : N
    ...
    [Q5     1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(1)*z1' + zeta(:,i)')*A5)' ; 1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(1)*z1' + zeta(:,i)')*A5) s51(i)]>=0 ;
    ...
    [Q5     1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(2)*z2' + zeta(:,i)')*A5)' ; 1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(2)*z2' + zeta(:,i)')*A5) s52(i)]>=0 ;
    ...
    [Q5     1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(3)*z3' + zeta(:,i)')*A5)' ; 1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(3)*z3' + zeta(:,i)')*A5) s53(i)]>=0 ;
    ...
    [Q5     1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(4)*z4' + zeta(:,i)')*A5)' ; 1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(4)*z4' + zeta(:,i)')*A5) s54(i)]>=0 ;
    ...
    [Q5     1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(5)*z5' + zeta(:,i)')*A5)' ; 1/2*(q(m1+m2+m3+m4+1:m1+m2+m3+m4+m5)+(- alpha_k(5)*z5' + zeta(:,i)')*A5) s55(i)]>=0 ;
