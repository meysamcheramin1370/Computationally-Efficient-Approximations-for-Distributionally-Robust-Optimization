function [ f_opt,X_opt,z_opt,CPUTime] = NewUB2CombinedProductionExp23( ZIGMAtemp,mu,m1,m2,m3,m4,N,switch1,R0,gamma2,cosiPrime,c,d,alpha_k,beta_k,m,n)
...
variables zeta(size,N) y(N) s11(N) s21(N) s31(N) s41(N) s12(N) s22(N) s32(N) s42(N) s13(N) s23(N) s33(N) s43(N) s14(N) s24(N) s34(N) s44(N) s15(N) s25(N) s35(N) s45(N);
...
%%
minimize( c'*x + Landa*R0 + gamma2*(sum(sum(eye(m1).*Q1)) + sum(sum(eye(m2).*Q2)) + sum(sum(eye(m3).*Q3)) + sum(sum(eye(m4).*Q4))) + (1/N) * sum(y) );
%%
subject to
