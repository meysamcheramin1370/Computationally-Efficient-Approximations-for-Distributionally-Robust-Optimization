...
gamma2 = 2;
R0_list = [20,100,200,500,1000]; % distance in Waserestein metric (Gao's paper - Example7)
...
for iterationNumber = 1 : Number
   ...
   for switch4 = 1 : 5
        ...
        for switch3 = 1 : 1
            ...
            [ f_opt2,X_opt2,z_opt2,CPUTime2,zetaOpt] = LBCombinedProduction(ZIGMAtemp,mu,m1,N,switch1,R0,gamma2,cosiPrime,c,d,alpha_k,beta_k,m,n);
            record(ID,7) = f_opt2;
            record(ID,8) = CPUTime2;

            record(ID,9) = abs(((f_opt1 - f_opt2)/ f_opt1) * 100); % Relative Gap

            TheoreticalGap = TheoGapCombinedProduction(z_opt2,zetaOpt,N,ZIGMAtemp,m1,size,switch1,f_opt1,gamma2,alpha_k);
