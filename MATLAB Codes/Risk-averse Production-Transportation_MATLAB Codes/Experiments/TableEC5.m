...
gamma1_list = [0.4,0.6,0.8];
gamma2 = 2;
R0_list=[30, 40, 50]; % distance in Waserestein metric (Gao's paper - Example7)
...
record = zeros (500,10);% 1=ID 2=m1  3=case number 4= original Value  5=original CPU  6= LB Value 7= LB CPU  8= Relative gap  9= Relative Theoretical gap
...
for iterationNumber = 1 : Number
    ...
    for j=1:3
        for i=1:3
            ...
            record(ID,5) = iterationNumber;
            record(ID,6) = f_opt1;
            record(ID,7) = CPUTime1;
            ...
            for switch3 = 1 : 5
                ...
                [ f_opt2,X_opt2,z_opt2,CPUTime2,zetaOpt] = WrittenExamLBCombinedProduction( ZIGMAtemp,mu,m1,N,switch1,R0,gamma1,gamma2,cosiPrime,c,d,alpha_k,beta_k,m,n);
                record(ID,8) = f_opt2;
                record(ID,9) = CPUTime2;
                
                record(ID,10) = abs(((f_opt1 - f_opt2)/ f_opt1) * 100); % Relative Gap
