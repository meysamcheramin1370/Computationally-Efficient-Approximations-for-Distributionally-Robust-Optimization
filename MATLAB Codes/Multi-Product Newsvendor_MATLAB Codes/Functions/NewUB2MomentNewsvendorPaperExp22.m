function [ f_opt,X_opt,CPUTime ] = NewUB2MomentNewsvendorPaperExp22( ZIGMAtemp,mu,A,b,m1,m2,m3,gamma1,gamma2,switch1,c,v,g)
...
cvx_begin sdp
                ...
                variables q(m) s s11 s21 s31 s12 s22 s32;
                ...
                %%
                minimize( s + sum(sum((gamma2*eye(m1)).*Q1))+ sum(sum((gamma2*eye(m2)).*Q2))+ sum(sum((gamma2*eye(m3)).*Q3)) + sqrt(gamma1)*norm(q,2) );
                %%
                subject to
                ...
                s11 + s21 + s31 == s-(c-v)'*X-(Landa1')*(b-A*mu);
                s12 + s22 + s32 == s-((c-v)'+(v-g)')*X-Landa2'*(b-A*mu)+(v-g)'*mu;              
