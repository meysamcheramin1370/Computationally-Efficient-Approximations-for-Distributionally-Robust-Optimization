function [ f_opt,X_opt,CPUTime] = GeneralNewUB2CombinedNewsvendorPaperExp21( ZIGMAtemp,mu,m1,m2,N,switch1,R0,cosiPrime,c,v,g,gamma2)
...
cvx_begin sdp
                ...
                %%
                minimize( Landa*R0 + gamma2 * (sum(sum(eye(m1).*Q1)) + sum(sum(eye(m2).*Q2))) + (1/N) * sum(y) );
                %%
                subject to
