...
%%%%%%%%%%%%%%%%%%%%% To solve general moment-based LB of Newsvendor problem %%%%%%%
...
for switch2 = 1 : 3
    ...
    for iterationNumber = 1 : Number
        ...
        for switch3 = 1 : 5
            ...
            [ f_opt2,X_opt2,CPUTime2] = UB1MomentNewsvendorPaper( ZIGMAtemp,mu,m1,A,b,gamma1,gamma2,switch1,c,v,g);
            record(ID,7) = f_opt2;
            record(ID,8) = CPUTime2;
            record(ID,9) = abs(((f_opt2 - f_opt1 )/ f_opt1) * 100); % Relative Gap
            
            [ f_opt3,X_opt3,CPUTime3] = NewUB2MomentNewsvendorPaper( ZIGMAtemp,mu,A,b,m1,gamma1,gamma2,switch1,c,v,g);
            record(ID,10) = f_opt3;
